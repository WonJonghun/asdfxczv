		...
		if (data.length != 0) {
			...
			for (let i = 0; i < data.length; i++) {
                ...
                kakao.maps.event.addListener(marker[i], 'click', function () {
                    ...
                    $('#input_build').val(data[i]['id']);
                    panel_set(building_id);
...
$('#input_build').on('change', function() {
    var building_id = $(this).val();
    panel_set(building_id);
})

$('#input_pannel').on('change', function() {
    var panel_id = $(this).val();
    cam_set(panel_id)
})


function panel_set(id) {
    $.ajax({
        type: "POST",
        url: "/get_panels",
        async: false,
        ...
        success: function (data) {
            if(data.length != 0) {
                $('#input_pannel').empty();
                $('#input_pannel').append('<option value="none">반을 선택해주세요.</option>');
                for(var i=0; i<data.length; i++) {
                    $('#input_pannel').append('<option value="'+data[i]['id']+'">'+data[i]['name']+'</option>')
                }
            }
        },
        error: function (e) {
...
function cam_set(id) {
    $.ajax({
        type: "POST",
        url: "manageStatic/get_camera_by_id",
        async: false,
        dataType: "json",
        data: {
            id: id
        ...
        success: function (data) {
            if(data.length != 0) {
                $('#input_cam').empty();
                $('#input_cam').append('<option value="none">카메라를 선택해주세요.</option>');
                for(var i=0; i<data.length; i++) {
